...
public class UserInteractionServiceTests
{
    ...
    private readonly DbContextOptions<SeekContext> _dbContextOptions;

    public UserInteractionServiceTests()
    {
        // Setup in-memory database
        _dbContextOptions = new DbContextOptionsBuilder<SeekContext>()
            .UseNpgsql("SeekTestDb")
            .Options;

        var context = new SeekContext(_dbContextOptions);
    ...
    public async Task SaveUserInteractionAsync_SavesInteraction()
    {
        ...
        // Assert
        using (var context = new SeekContext(_dbContextOptions))
        {
